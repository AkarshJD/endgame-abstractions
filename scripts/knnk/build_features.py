"""
Extract geometric features for all KNNK positions.

Subsamples 2M positions from the full dataset to keep memory manageable.
Uses multiprocessing for parallel FEN parsing + feature extraction.
...
from endgame.features.knnk_geom import compute_knnk_features


RAW_PATH    = "data/raw/knnk/full.csv"
OUTPUT_PATH = "data/processed/knnk/features.csv"
...
def process_chunk(rows):
    ...
    for fen, wdl in rows:
        try:
            ...
            features = compute_knnk_features(board)
            features["fen"] = fen
            features["wdl"] = wdl
            out.append(features)
        except Exception:
